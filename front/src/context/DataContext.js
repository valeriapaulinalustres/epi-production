...
let ab = [...a, ...b]
  ...
  function calcularDescartadosDengue() {
    let a = baseCompleta.filter(el => el.CLASIFICACION_MANUAL == "Caso descartado por diagnóstico diferencial" && el.DEPARTAMENTO_RESIDENCIA == "Morón" )
      let b = baseCompleta.filter(el => el.CLASIFICACION_MANUAL == "Caso descartado" && el.DEPARTAMENTO_RESIDENCIA == "Morón" && el.EVENTO == "Dengue" )
      let c = baseCompleta.filter(el => el.CLASIFICACION_MANUAL == "Caso descartado por epidemiología" && el.DEPARTAMENTO_RESIDENCIA == "Morón" && el.EVENTO == "Dengue" )
  ...
  function calcularDescartadosDengueEntreFechas(fechaInicio, fechaFin) {
    let a = baseCompleta.filter(el => el.CLASIFICACION_MANUAL == "Caso descartado por diagnóstico diferencial" && el.DEPARTAMENTO_RESIDENCIA == "Morón" && el.EVENTO == "Dengue" && el.SEPI_APERTURA >= fechaInicio && el.SEPI_APERTURA <= fechaFin)
      let b = baseCompleta.filter(el => el.CLASIFICACION_MANUAL == "Caso descartado" && el.DEPARTAMENTO_RESIDENCIA == "Morón" && el.EVENTO == "Dengue" && el.SEPI_APERTURA >= fechaInicio && el.SEPI_APERTURA <= fechaFin)
      let c = baseCompleta.filter(el => el.CLASIFICACION_MANUAL == "Caso descartado por epidemiología" && el.DEPARTAMENTO_RESIDENCIA == "Morón" && el.EVENTO == "Dengue" && el.SEPI_APERTURA >= fechaInicio && el.SEPI_APERTURA <= fechaFin)
let abc = [...a, ...b, ...c]
      return abc
  }

  function calcularSospechososDengue() {
    let a = baseCompleta.filter(el => el.CLASIFICACION_MANUAL == "Caso sospechoso" && el.DEPARTAMENTO_RESIDENCIA == "Morón" && el.EVENTO == "Dengue" )
      let b = baseCompleta.filter(el => el.CLASIFICACION_MANUAL == "Caso sospechoso no conclusivo" && el.DEPARTAMENTO_RESIDENCIA == "Morón" && el.EVENTO == "Dengue" )
  ...
  function calcularSospechososDengueEntreFechas(fechaInicio, fechaFin) {
    let a = baseCompleta.filter(el => el.CLASIFICACION_MANUAL == "Caso sospechoso" && el.DEPARTAMENTO_RESIDENCIA == "Morón" && el.EVENTO == "Dengue"  && el.SEPI_APERTURA >= fechaInicio && el.SEPI_APERTURA <= fechaFin)
      let b = baseCompleta.filter(el => el.CLASIFICACION_MANUAL == "Caso sospechoso no conclusivo" && el.DEPARTAMENTO_RESIDENCIA == "Morón" && el.EVENTO == "Dengue"  && el.SEPI_APERTURA >= fechaInicio && el.SEPI_APERTURA <= fechaFin)
  ...
  function calcularEventoEnEmbarazoEntreFechas(evento, fechaInicio, fechaFin) {
    return baseCompleta.filter(el => el.EVENTO == evento && el.EMBARAZADA == "SI" && el.DEPARTAMENTO_RESIDENCIA == "Morón" && el.SEPI_APERTURA >= fechaInicio && el.SEPI_APERTURA <= fechaFin)
  }
  ...
  function calcularConfirmadosEmbarazoDengue() {
    let a = baseCompleta.filter(el => el.CLASIFICACION_MANUAL == "Caso confirmado DEN-1" && el.DEPARTAMENTO_RESIDENCIA == "Morón" && el.EMBARAZADA == "SI" )
      let b =baseCompleta.filter(el => el.CLASIFICACION_MANUAL == "Caso confirmado sin serotipo" && el.DEPARTAMENTO_RESIDENCIA == "Morón" && el.EMBARAZADA == "SI" )
  ...
  function calcularConfirmadosEmbarazoDengueEntreFechas(fechaInicio, fechaFin) {
    let a = baseCompleta.filter(el => el.CLASIFICACION_MANUAL == "Caso confirmado DEN-1" && el.DEPARTAMENTO_RESIDENCIA == "Morón" && el.EMBARAZADA == "SI" && el.SEPI_APERTURA >= fechaInicio && el.SEPI_APERTURA <= fechaFin)
      let b =baseCompleta.filter(el => el.CLASIFICACION_MANUAL == "Caso confirmado sin serotipo" && el.DEPARTAMENTO_RESIDENCIA == "Morón" && el.EMBARAZADA == "SI" && el.SEPI_APERTURA >= fechaInicio && el.SEPI_APERTURA <= fechaFin)
  ...
  function calcularDescartadosEmbarazoDengue() {
    let a = baseCompleta.filter(el => el.CLASIFICACION_MANUAL == "Caso descartado por diagnóstico diferencial" && el.DEPARTAMENTO_RESIDENCIA == "Morón" && el.EMBARAZADA == "SI")
    let b =baseCompleta.filter(el => el.CLASIFICACION_MANUAL == "Caso descartado por diagnóstico diferencial" && el.DEPARTAMENTO_RESIDENCIA == "Morón" && el.EMBARAZADA == "SI")
    let c =baseCompleta.filter(el => el.CLASIFICACION_MANUAL == "Caso descartado por epidemiología" && el.DEPARTAMENTO_RESIDENCIA == "Morón" && el.EMBARAZADA == "SI")
    let d =baseCompleta.filter(el => el.CLASIFICACION_MANUAL == "Caso descartado" && el.DEPARTAMENTO_RESIDENCIA == "Morón" && el.EMBARAZADA == "SI")
  ...
  function calcularDescartadosEmbarazoDengueEntreFechas(fechaInicio, fechaFin) {
    let a = baseCompleta.filter(el => el.CLASIFICACION_MANUAL == "Caso descartado por diagnóstico diferencial" && el.DEPARTAMENTO_RESIDENCIA == "Morón" && el.EMBARAZADA == "SI"&& el.SEPI_APERTURA >= fechaInicio && el.SEPI_APERTURA <= fechaFin)
    let b =baseCompleta.filter(el => el.CLASIFICACION_MANUAL == "Caso descartado por diagnóstico diferencial" && el.DEPARTAMENTO_RESIDENCIA == "Morón" && el.EMBARAZADA == "SI"&& el.SEPI_APERTURA >= fechaInicio && el.SEPI_APERTURA <= fechaFin)
    let c =baseCompleta.filter(el => el.CLASIFICACION_MANUAL == "Caso descartado por epidemiología" && el.DEPARTAMENTO_RESIDENCIA == "Morón" && el.EMBARAZADA == "SI"&& el.SEPI_APERTURA >= fechaInicio && el.SEPI_APERTURA <= fechaFin)
  ...
  function calcularTotalNotificadosClinicaEntreFechas(enfermedad, fechaInicio, fechaFin) {
    return baseCompletaClinica.filter(el => el.NOMBREEVENTOAGRP == enfermedad && el.SEMANA >= fechaInicio && el.SEMANA <= fechaFin )
  }

  function calcularNotificadosXSEClinica (arr,se) {
    return arr.filter(el => el.DEPARTAMENTO == "MORÓN" && el.SEMANA == se).length || 0
  ...
  function quitarDuplicados(arr) {
    ...
    for (let i = 0; i < arr.length; i++) {
        ...
        if (filtrado.length == 0) { sinDuplicados.push(el1) }
    }
//console.log(sinDuplicados);
  ...
  let arrayTotalNotificadosSifilisCongenita = calcularTotalNotificados("Sífilis congénita");
  arrayTotalNotificadosSifilisCongenita = quitarDuplicados(arrayTotalNotificadosSifilisCongenita);
  let arrayTotalNotificadosSifilisEmbarazadas = calcularTotalNotificados("Sífilis en personas gestantes");
  arrayTotalNotificadosSifilisEmbarazadas = quitarDuplicados(arrayTotalNotificadosSifilisEmbarazadas);

  let arrayTotalGeneralNotificadosSifilis = [...arrayTotalNotificadosSifilis, ...arrayTotalNotificadosSifilisCongenita, ...arrayTotalNotificadosSifilisEmbarazadas];
 ...
 arrayTotalGeneralNotificadosSifilis = quitarDuplicados(arrayTotalGeneralNotificadosSifilis)  
  ...
  const numeroTotalNotificadosSifilisEmbarazadas = arrayTotalNotificadosSifilisEmbarazadas.length || 0;

  const numeroTotalGeneralNotificadosSifilis = arrayTotalGeneralNotificadosSifilis.length || 0;
  ...
  //confirmados
let arrayConfirmadosTotalGeneralSifilis = [...calcularConfirmadosPorClasificacion("Caso confimado en banco de sangre"), ...calcularConfirmadosPorClasificacion('Caso confirmado de Sífilis sin especificar'), ...calcularConfirmadosPorClasificacion('Caso confirmado de Sífilis temprana'), ...calcularConfirmadosPorClasificacion('Caso de Sífilis congénita confirmada por laboratorio'), ...calcularConfirmadosPorClasificacion('Caso confirmado de Sífilis'), ...calcularConfirmadosPorClasificacion('Caso confirmado por criterio epidemiológico')];

arrayConfirmadosTotalGeneralSifilis = arrayConfirmadosTotalGeneralSifilis.filter(el=>el.EVENTO === 'Sífilis' || el.EVENTO === 'Sífilis en personas gestantes' || el.EVENTO === 'Sífilis congénita')

arrayConfirmadosTotalGeneralSifilis = quitarDuplicados(arrayConfirmadosTotalGeneralSifilis)
const numeroConfirmadosTotalGeneralSifilis = arrayConfirmadosTotalGeneralSifilis.length || 0;


  //probables
let arrayProbablesTotalGeneralSifilis = [...calcularConfirmadosPorClasificacion('Caso probable de Sífilis sin especificar estadío'), ...calcularConfirmadosPorClasificacion('Caso probable de Sífilis temprana'), ...calcularConfirmadosPorClasificacion('Caso probable en banco de sangre'), ...calcularConfirmadosPorClasificacion('Caso probable de sífilis')];

arrayProbablesTotalGeneralSifilis = arrayProbablesTotalGeneralSifilis.filter(el=>el.EVENTO === 'Sífilis' || el.EVENTO === 'Sífilis en personas gestantes' || el.EVENTO === 'Sífilis congénita')

arrayProbablesTotalGeneralSifilis = quitarDuplicados(arrayProbablesTotalGeneralSifilis);

const numeroProbablesTotalGeneralSifilis =  arrayProbablesTotalGeneralSifilis.length || 0;

  //descartados
let arrayDescartadosTotalGeneralSifilis = calcularConfirmadosPorClasificacion('Caso descartado de Sífilis')
arrayDescartadosTotalGeneralSifilis = quitarDuplicados(arrayDescartadosTotalGeneralSifilis);

const numeroDescartadosTotalGeneralSifilis = arrayDescartadosTotalGeneralSifilis.length || 0;
...
arrayTotalGeneralSifilisMoron = quitarDuplicados(arrayTotalGeneralSifilisMoron)
  const numeroTotalGeneralSifilisMoron = arrayTotalGeneralSifilisMoron.length || 0;


  let arrayTotalGeneralSifilisNoMoron = [...calcularDptoCargaNoMoron("Sífilis"), ...calcularDptoCargaNoMoron("Sífilis congénita"), ...calcularDptoCargaNoMoron("Sífilis en personas gestantes")]
  arrayTotalGeneralSifilisNoMoron = quitarDuplicados(arrayTotalGeneralSifilisNoMoron);
  const numeroTotalGeneralSifilisNoMoron = arrayTotalGeneralSifilisNoMoron.length || 0;
...
arrayTotalGeneralSifilisMoronEntreFechas = quitarDuplicados(arrayTotalGeneralSifilisMoronEntreFechas)
 ...
 let arrayConfirmadosFemeninosSifilis = arrayConfirmadosTotalGeneralSifilis.filter(el=>el.SEXO === "F")
const numeroConfirmadosFemeninosSifilis = arrayConfirmadosFemeninosSifilis.length || 0;

let arrayConfirmadosMasculinosSifilis = arrayConfirmadosTotalGeneralSifilis.filter(el=>el.SEXO === "M")
const numeroConfirmadosMasculinosSifilis = arrayConfirmadosMasculinosSifilis.length || 0;


let arrayConfirmadosSDSifilis = arrayConfirmadosTotalGeneralSifilis.filter(el=>el.SEXO === "A" || el.SEXO === "NA")
const numeroConfirmadosSDSifilis = arrayConfirmadosSDSifilis.length || 0;
 ...
 let arrayProbablesFemeninosSifilis = arrayProbablesTotalGeneralSifilis.filter(el=>el.SEXO === "F")
 const numeroProbablesFemeninosSifilis = arrayProbablesFemeninosSifilis.length || 0;

 let arrayProbablesMasculinosSifilis = arrayProbablesTotalGeneralSifilis.filter(el=>el.SEXO === "M")
 const numeroProbablesMasculinosSifilis = arrayProbablesMasculinosSifilis.length || 0;

 let arrayProbablesSDSifilis = arrayProbablesTotalGeneralSifilis.filter(el=>el.SEXO === "A" || el.SEXO === "NA")
...
const sifilisXse = [
  ...
  let arrayTotalNotificadosHivPerinatal = calcularTotalNotificados("VIH - Expuesto perinatal");
  arrayTotalNotificadosHivPerinatal = quitarDuplicados(arrayTotalNotificadosHivPerinatal)
  let arrayTotalNotificadosHivEmbarazo = calcularTotalNotificados("VIH en embarazo");
  arrayTotalNotificadosHivEmbarazo = quitarDuplicados(arrayTotalNotificadosHivEmbarazo)

  let arrayTotalGeneralNotificadosHiv = [...arrayTotalNotificadosHiv, ...arrayTotalNotificadosHivPerinatal, ...arrayTotalNotificadosHivEmbarazo]
  arrayTotalGeneralNotificadosHiv = quitarDuplicados(arrayTotalGeneralNotificadosHiv)
  ...
  const numeroTotalNotificadosHivEmbarazo = arrayTotalNotificadosHivEmbarazo.length || 0;
  ...
  //---------------clasificaciones totales
  //confirmados
let arrayConfirmadosTotalGeneralHiv = calcularConfirmadosPorClasificacion('Caso confirmado de VIH')
arrayConfirmadosTotalGeneralHiv = quitarDuplicados(arrayConfirmadosTotalGeneralHiv)
const numeroConfirmadosTotalGeneralHiv = arrayConfirmadosTotalGeneralHiv.length || 0;

  //probables
   let numeroProbablesTotalGeneralHiv = [...calcularTotalNotificados("VIH"), ...calcularTotalNotificados("VIH - Expuesto perinatal"), ...calcularTotalNotificados("VIH en embarazo")].filter(el=> el.CLASIFICACION_MANUAL === "Caso probable de infección por VIH").length || 0

  //descartados
let arrayDescartadosTotalGeneralHiv = calcularConfirmadosPorClasificacion('Caso descartado de VIH')
arrayDescartadosTotalGeneralHiv = quitarDuplicados(arrayDescartadosTotalGeneralHiv)
const numeroDescartadosTotalGeneralHiv = arrayDescartadosTotalGeneralHiv.length || 0
...
let arrayTotalGeneralHivNoMoron = [...calcularDptoCargaNoMoron("VIH"), ...calcularDptoCargaNoMoron("VIH - Expuesto perinatal"), ...calcularDptoCargaNoMoron("VIH en embarazo")]
...
//entre fechas
  ...
  //---------------clasificaciones totales
  //confirmados
let arrayBaciloscopiaPositiva = calcularConfirmadosPorClasificacion("Baciloscopía positiva");
let arrayComplejoTBC = calcularConfirmadosPorClasificacion("Complejo Mycobacterium tuberculosis");
let arrayMicobacterium = calcularConfirmadosPorClasificacion("Mycobacterium tuberculosis");
let arrayHistopatologiaSugestiva = calcularConfirmadosPorClasificacion("Histopatologia sugestiva");
...
const numeroTotalNotificadosDengueEntreFechas = arrayTotalNotificadosDengueEntreFechas.length || 0;
  ...
  //entre fechas
  const numeroTotalNotificadosDengueFemeninoEntreFechas = calcularPorSexo(arrayTotalNotificadosDengueEntreFechas, "F");
  const numeroTotalNotificadosDengueMasculinoEntreFechas = calcularPorSexo(arrayTotalNotificadosDengueEntreFechas, "M");
  const numeroTotalNotificadosDengueSdEntreFechas = calcularPorSexo(arrayTotalNotificadosDengueEntreFechas, "NA")
  ...
  //probables
  let arrayProbablesTotalDengue = calcularClasificacionManualPorEvento("Dengue", "Caso probable")
...
//entre fechas
//confirmados
let arrayConfirmadosTotalDengueEntreFechas = calcularConfirmadosDengueEntreFechas(semanaInicial, semanaFinal)
  arrayConfirmadosTotalDengueEntreFechas = quitarDuplicados(arrayConfirmadosTotalDengueEntreFechas)
  let numeroConfirmadosTotalDengueEntreFechas = arrayConfirmadosTotalDengueEntreFechas.length || 0

//probables

let arrayProbablesTotalDengueEntreFechas = calcularClasificacionManualPorEventoEntreFechas("Dengue", "Caso probable", semanaInicial, semanaFinal)
arrayProbablesTotalDengueEntreFechas = quitarDuplicados(arrayProbablesTotalDengueEntreFechas)
let numeroProbablesTotalDengueEntreFechas = arrayProbablesTotalDengueEntreFechas.length || 0;

//descartados
let arrayDescartadosTotalDengueEntreFechas = calcularDescartadosDengueEntreFechas(semanaInicial, semanaFinal)
arrayDescartadosTotalDengueEntreFechas = quitarDuplicados(arrayDescartadosTotalDengueEntreFechas)
let numeroDescartadosTotalDengueEntreFechas = arrayDescartadosTotalDengueEntreFechas.length || 0



//sospechosos
let arraySospechososTotalDengueEntreFechas = calcularSospechososDengueEntreFechas(semanaInicial, semanaFinal)
  arraySospechososTotalDengueEntreFechas = quitarDuplicados(arraySospechososTotalDengueEntreFechas)
  let numeroSospechososTotalDengueEntreFechas = arraySospechososTotalDengueEntreFechas.length || 0


  //embarazadas
  let arrayEmbarazadasNotificadoTotalDengue = calcularEventoEnEmbarazo("Dengue");
  arrayEmbarazadasNotificadoTotalDengue = quitarDuplicados(arrayEmbarazadasNotificadoTotalDengue)
let numeroEmbarazadasNotificadoTotalDengue = arrayEmbarazadasNotificadoTotalDengue.length || 0;

  let arrayEmbarazadasConfirmadasDengue = calcularConfirmadosEmbarazoDengue();
  arrayEmbarazadasConfirmadasDengue = quitarDuplicados(arrayEmbarazadasConfirmadasDengue)
let numeroEmbarazadasConfirmadasDengue = arrayEmbarazadasConfirmadasDengue.length || 0;

  let arrayEmbarazadasDescartadasDengue = calcularDescartadosEmbarazoDengue()
  arrayEmbarazadasDescartadasDengue = quitarDuplicados(arrayEmbarazadasDescartadasDengue)
  let numeroEmbarazadasDescartadasDengue = arrayEmbarazadasDescartadasDengue.length || 0;

  //embarazadas entre fechas
  let arrayEmbarazadasNotificadoTotalDengueEntreFechas = calcularEventoEnEmbarazoEntreFechas("Dengue", semanaInicial, semanaFinal);
  arrayEmbarazadasNotificadoTotalDengueEntreFechas = quitarDuplicados(arrayEmbarazadasNotificadoTotalDengueEntreFechas)
let numeroEmbarazadasNotificadoTotalDengueEntreFechas = arrayEmbarazadasNotificadoTotalDengueEntreFechas.length || 0;

  let arrayEmbarazadasConfirmadasDengueEntreFechas = calcularConfirmadosEmbarazoDengueEntreFechas(semanaInicial, semanaFinal);
  arrayEmbarazadasConfirmadasDengueEntreFechas = quitarDuplicados(arrayEmbarazadasConfirmadasDengueEntreFechas)
let numeroEmbarazadasConfirmadasDengueEntreFechas = arrayEmbarazadasConfirmadasDengueEntreFechas.length || 0;

  let arrayEmbarazadasDescartadasDengueEntreFechas = calcularDescartadosEmbarazoDengueEntreFechas(semanaInicial, semanaFinal)
  arrayEmbarazadasDescartadasDengueEntreFechas = quitarDuplicados(arrayEmbarazadasDescartadasDengueEntreFechas)
  let numeroEmbarazadasDescartadasDengueEntreFechas = arrayEmbarazadasDescartadasDengueEntreFechas.length || 0;
  ...
  arrayTotalGeneralDengueNoMoron = quitarDuplicados(arrayTotalGeneralDengueNoMoron)
  let numeroTotalGeneralDengueNoMoron = arrayTotalGeneralDengueNoMoron.length || 0 ;
  ...
  //-------------Departamento de carga entre fechas
  let arrayTotalGeneralDengueMoronEntreFechas = calcularDptoCargaMoronEntreFechas("Dengue", semanaInicial, semanaFinal)
  arrayTotalGeneralDengueMoronEntreFechas = quitarDuplicados(arrayTotalGeneralDengueMoronEntreFechas)
  let numeroTotalGeneralDengueMoronEntreFechas = arrayTotalGeneralDengueMoronEntreFechas.length || 0;

  let arrayTotalGeneralDengueNoMoronEntreFechas = calcularDptoCargaNoMoronEntreFechas("Dengue", semanaInicial, semanaFinal);
  arrayTotalGeneralDengueNoMoronEntreFechas = quitarDuplicados(arrayTotalGeneralDengueNoMoronEntreFechas)
  let numeroTotalGeneralDengueNoMoronEntreFechas = arrayTotalGeneralDengueNoMoronEntreFechas.length || 0 ;
...
const totalNotificadosETI = calcularTotalNotificadosClinica("Enfermedad tipo influenza (ETI)").length || 0;
const arrayTotalNotificadosETI = calcularTotalNotificadosClinica("Enfermedad tipo influenza (ETI)")

//entre fechas
const totalNotificadosETIEntreFechas = calcularTotalNotificadosClinicaEntreFechas("Enfermedad tipo influenza (ETI)", semanaInicial, semanaFinal).length || 0
...
let arrayConfirmadosCovidPcr = calcularClasificacionManualPorEvento("COVID-19, Influenza y OVR en ambulatorios (No UMAs)", "SARS-COV-2 positivo por métodos moleculares")
arrayConfirmadosCovidPcr = quitarDuplicados(arrayConfirmadosCovidPcr)

let arrayConfirmadosCovidAg = calcularClasificacionManualPorEvento("COVID-19, Influenza y OVR en ambulatorios (No UMAs)", "SARS-COV-2 positivo por test de Ag")
arrayConfirmadosCovidAg = quitarDuplicados(arrayConfirmadosCovidAg)

let arrayConfirmadosCovidAutotest = calcularClasificacionManualPorEvento("COVID-19, Influenza y OVR en ambulatorios (No UMAs)", "Positivo por autotest")
arrayConfirmadosCovidAutotest = quitarDuplicados(arrayConfirmadosCovidAutotest)

let arrayConfirmadosCovidTotal = [...arrayConfirmadosCovidPcr, ...arrayConfirmadosCovidAg, ...arrayConfirmadosCovidAutotest]
...
let numeroConfirmadosCovidAutotest = arrayConfirmadosCovidAutotest.length || 0

let numeroConfirmadosCovidTotal = arrayConfirmadosCovidTotal.length || 0

//Entre fechas:
let arrayConfirmadosCovidPcrEntreFechas = calcularClasificacionManualPorEventoEntreFechas("COVID-19, Influenza y OVR en ambulatorios (No UMAs)", "SARS-COV-2 positivo por métodos moleculares", semanaInicial, semanaFinal)
arrayConfirmadosCovidPcrEntreFechas = quitarDuplicados(arrayConfirmadosCovidPcrEntreFechas)

let arrayConfirmadosCovidAgEntreFechas = calcularClasificacionManualPorEventoEntreFechas("COVID-19, Influenza y OVR en ambulatorios (No UMAs)", "SARS-COV-2 positivo por test de Ag", semanaInicial, semanaFinal)
arrayConfirmadosCovidAgEntreFechas = quitarDuplicados(arrayConfirmadosCovidAgEntreFechas)

let arrayConfirmadosCovidAutotestEntreFechas = calcularClasificacionManualPorEventoEntreFechas("COVID-19, Influenza y OVR en ambulatorios (No UMAs)", "Positivo por autotest", semanaInicial, semanaFinal)
arrayConfirmadosCovidAutotestEntreFechas = quitarDuplicados(arrayConfirmadosCovidAutotestEntreFechas)

let arrayConfirmadosCovidTotalEntreFechas = [...arrayConfirmadosCovidPcrEntreFechas, ...arrayConfirmadosCovidAgEntreFechas, ...arrayConfirmadosCovidAutotestEntreFechas]
arrayConfirmadosCovidTotalEntreFechas = quitarDuplicados(arrayConfirmadosCovidTotalEntreFechas)

let numeroConfirmadosCovidPcrEntreFechas = arrayConfirmadosCovidPcrEntreFechas.length || 0
let numeroConfirmadosCovidAgEntreFechas = arrayConfirmadosCovidAgEntreFechas.length || 0
let numeroConfirmadosCovidAutotestEntreFechas = arrayConfirmadosCovidAutotestEntreFechas.length || 0


let numeroConfirmadosCovidTotalEntreFechas = arrayConfirmadosCovidTotalEntreFechas.length || 0

//Por sexo
let numeroConfirmadosFemeninosCovid = calcularPorSexo(arrayConfirmadosCovidTotal, "F")
let numeroConfirmadosMasculinosCovid = calcularPorSexo(arrayConfirmadosCovidTotal, "M")
let numeroConfirmadosSdCovid = calcularPorSexo(arrayConfirmadosCovidTotal, "NA")


  //--Departamento de carga
let numeroConfirmadosCovidTotalMoron = arrayConfirmadosCovidTotal.filter(el=> el.DEPARTAMENTO_CARGA === "Morón").length || 0
let numeroConfirmadosCovidTotalNoMoron = arrayConfirmadosCovidTotal.filter(el=> el.DEPARTAMENTO_CARGA !== "Morón").length || 0

  const porcentajeNotificadosCovidMoron = Math.round(numeroConfirmadosCovidTotalMoron / (numeroConfirmadosCovidTotalNoMoron + numeroConfirmadosCovidTotalMoron) * 100) || 0

//Entre fechas
let numeroConfirmadosCovidTotalMoronEntreFechas = arrayConfirmadosCovidTotalEntreFechas.filter(el=> el.DEPARTAMENTO_CARGA === "Morón").length || 0
let numeroConfirmadosCovidTotalNoMoronEntreFechas = arrayConfirmadosCovidTotalEntreFechas.filter(el=> el.DEPARTAMENTO_CARGA !== "Morón").length || 0

  const porcentajeNotificadosCovidMoronEntreFechas = Math.round(numeroConfirmadosCovidTotalMoronEntreFechas / (numeroConfirmadosCovidTotalNoMoronEntreFechas + numeroConfirmadosCovidTotalMoronEntreFechas) * 100) || 0
 ...
 const covidXse = [
  ...
  //-------------------INFLUENZA-----------------------------
  //Totales
let arrayConfirmadosInfluenzaA = calcularClasificacionManualPorEvento("COVID-19, Influenza y OVR en ambulatorios (No UMAs)", "Influenza A - Positivo")
arrayConfirmadosInfluenzaA = quitarDuplicados(arrayConfirmadosInfluenzaA)

let arrayConfirmadosInfluenzaB = calcularClasificacionManualPorEvento("COVID-19, Influenza y OVR en ambulatorios (No UMAs)", "Influenza B - Positivo")

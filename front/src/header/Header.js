...
function Header() {
  ...
  const [stateJoyride, setStateJoyride] = useState({
    ...
    steps: [
      ...
      },
      {
        content: (
          <h2>
            Primero haz click aquí para cargar los archivos y las fechas, los
            cuales permitirán generar los gráficos
          </h2>
        ),
        placement: 'bottom',
        target: '#upload',
      ...
      },
      {
        content: (
          <h2>
            Luego puedes elegir la enfermedad en el menú desplegable para ver su
  ...
  // *** Joyride ***
  useEffect(() => {
    if (!stateJoyride.localStorageKey) {
      setStateJoyride({ ...stateJoyride, run: true });
      return;
    }
    const tourViewed = window.localStorage.getItem(
        ...
        <div className='modal'>
          <div className='window'>
            ...
            <p className='modal-text'>
              El calendario epidemiológico comprende 52 semanas epidemiológicas
              que dividen los 365 días del año. Su uso durante las actividades
              de vigilancia es importante porque el estandardizar la variable
              tiempo, permite la comparación de eventos epidemiológicos
              sucedidos en determinado año o período dentro de un año con los de
              años previos o de otros países. (www3.paho.org)
            </p>
            <div className='epiCalendar-container'>
              <img
                src='/calendario-epidemiológico.png'
                alt='calendario epidemiológico'
                width='100%'
              ></img>
            </div>
          </div>
